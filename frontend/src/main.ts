...
const handleScrollDown = () => messageBox?.scrollTo({top: messageBox.scrollHeight, left: 0, behavior: "smooth"})

const handleFormSubmit = async () =>  {
  const formData = new FormData(form as HTMLFormElement)
  // Add the users chat
  messageBox?.append(generateChat(formData.get("message"), false, generateID()))
  ...
  const { data: text } = await request.json()
